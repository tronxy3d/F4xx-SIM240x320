...
void GcodeSuite::M600() {
  ...
  if (pause_print(retract, park_point, true, unload_length DXC_PASS)) {
    if (standardM600) {
      #if TRONXY_UI
      yevt.setEvt(TASK_LOOP,LOOPEVT_AFTER_UI_UPDATE,closePopupMessage);
      yevt.addEvt(TASK_SEQUENCE,popupFilamentInsert);
      #endif
  ...
  #if TRONXY_UI
  else if(messageSt.currentIndex == POPUP_WAITING) {
    closePopupMessage();
    tronxyUpdate();
  }
  #endif
